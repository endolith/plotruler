...
A translucent desktop overlay for reading exact (X, Y) values off a graph shown on screen.

The graph is whatever is running underneath — a browser, a PDF viewer, a plotting window. PlotRuler never renders the graph; it is a live see-through overlay. Calibrate once by clicking a couple of known points and typing their labels, then hover to read coordinates anywhere and click to copy.
